...
function [c,iflag] = Lsolvesolution(L,p,b);
%
%  solves  Lc = b where L is a (logically) unit lower triangular
...
%
%
%  Input:  L   an n by n array holding a lower
...
%
   ...
   c = b;
%
%  Foward substitution loop
%
   for j = 1:n-1,
   
       t = c(j); c(j) = c(p(j)); c(p(j)) = t;     % Apply Pj
       for i = j+1:n,

           c(i) = c(i) - L(i,j)*c(j);             % Apply Lj
       end
   end